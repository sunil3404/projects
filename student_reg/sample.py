...
class User(Base):
    ...
    post = relationship("Posts", passive_deletes=True, back_populates='author')
...
class Child(Base):
    ...
    cname = Column(String)
    p_id =  Column(Integer, ForeignKey("parent.parent_id", ondelete="CASCADE"))
...
class Parent(Base):
    ...
    children = relationship("Child", back_populates="parent", cascade="all,delete", passive_deletes=True)
