...
@router.post("")
def create_blogs(request: models.Blogs, db: Session = Depends(database.get_db)):
...
def show_blogs(id: int, response: Response, db: Session = Depends(database.get_db)):
...
def update_blog(id: int, request: models.Blogs, response : Response, db: Session = Depends(database.get_db)):
